...
static int vidioc_subscribe_event(struct v4l2_fh *fh,
					 ...
					 {
	struct sun6i_video *video = video_get_drvdata(fh->vdev);
	int ret = -EINVAL;
	struct v4l2_subdev *subdev = sun6i_video_remote_subdev(video, NULL);
	if(subdev){
		ret = v4l2_subdev_call(subdev, core, subscribe_event, fh, sub);
	}
	DEV_WARN("sun6i_video: %p result: %d", video, ret);
				...
				struct v4l2_format *fmt)
{
	...
	if (subdev){
		...
		r = v4l2_subdev_call(subdev, pad, get_fmt, NULL, &format);

		fmt->fmt.pix.width = format.format.width;
		fmt->fmt.pix.height = format.format.height;
		fmt->fmt.pix.pixelformat = V4L2_PIX_FMT_YUV420;
		fmt->fmt.pix.field = format.format.field;
		fmt->fmt.pix.colorspace = format.format.colorspace;
		fmt->fmt.pix.quantization = format.format.quantization;		
	}
...
int sun6i_video_get_lanes(struct sun6i_video *video){
	struct v4l2_mbus_config cfg = {};
	struct v4l2_subdev *subdev = sun6i_video_remote_subdev(video, NULL);
	int lanes = 4;
	DEV_WARN("");
	if (subdev){
		v4l2_subdev_call(subdev, pad, get_mbus_config, NULL, &cfg);
		lanes = cfg.flags;
		DEV_WARN("%d", lanes);
	}	
	DEV_WARN("");
