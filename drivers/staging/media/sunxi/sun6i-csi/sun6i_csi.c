...
{
	DEV_WARN("mbus_code: %04X pixformat: %08X", mbus_code, pixformat);
	/* non-YUV */
						 ...
						 u32 pixformat, u32 field)
{
	...
	DEV_WARN("field: %d pixformat: %08X", field, pixformat);
...
static void sun6i_csi_set_format(struct sun6i_csi_dev *sdev)
{
	...
	DEV_WARN("Input Format: %d", val);			   
	cfg |= CSI_CH_CFG_INPUT_FMT(val);

	val = get_csi_output_format(sdev, csi->config.pixelformat,
				    csi->config.field);

	DEV_WARN("Output Format: %d", val);				
	cfg |= CSI_CH_CFG_OUTPUT_FMT(val);

	val = get_csi_input_seq(sdev, csi->config.code,
				csi->config.pixelformat);

	DEV_WARN("Input Sequence: %d Field: %d", val, csi->config.field);
...
#else
	// dma_direct_set_offset(sdev->dev, PHYS_OFFSET >> PAGE_SHIFT, 0, SZ_4G);
#endif
