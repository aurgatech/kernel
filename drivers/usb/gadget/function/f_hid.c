...
#define CT_FUNC_HID_KB_IDX	0
#define CT_FUNC_HID_MS_IDX	1
#define CT_FUNC_HID_TS_IDX	2

static struct usb_string ct_func_string_defs[] = {
	[CT_FUNC_HID_KB_IDX].s	= "AURGA Keyboard",
	[CT_FUNC_HID_MS_IDX].s	= "AURGA Mouse",
	[CT_FUNC_HID_TS_IDX].s	= "AURGA Touch Screen",
	{},			/* end of list */
};
...
static void hidg_ssreport_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	report_id = ((char*)req->buf)[0];

	hidg->set_report_length = req->actual;
	//memcpy(hidg->set_report_buf, req->buf, req->actual);

	spin_unlock_irqrestore(&hidg->read_spinlock, flags);
	if(report_id == REPORT_ID_AURGA){
		aurga_set_report(req);
	}
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8
		...
		//dump_hex("req", &ctrl, sizeof(struct usb_ctrlreques));

		if(report_id == REPORT_ID_AURGA && length > 32){
			aurga_get_report(req, hidg);
		}

		if(ctrl->bRequestType == 0xA1 && ctrl->bRequest == 0x01 && value == 0x306){
			aurga_get_touch_06(req, hidg);
		}

		if(ctrl->bRequestType == 0xA1 && ctrl->bRequest == 0x01 && value == 0x305){
			aurga_get_touch_05(req, hidg);
		}
	...
	case ((USB_DIR_IN | USB_TYPE_STANDARD | USB_RECIP_INTERFACE) << 8
		...
		case HID_DT_REPORT:
			...
			if(hidg->report_desc_length == 646){
				compatible_buf = ((__u8*)req->buf);
				printk(KERN_WARNING"%s[%d] speed %d desc length %d report size %d\n", 
						__func__, __LINE__, 
						cdev->gadget->speed, hidg->report_desc_length, compatible_buf[642]);
				if(cdev->gadget->speed >= USB_SPEED_HIGH){
					compatible_buf[642] = 0xFF;
				}else{
					compatible_buf[642] = 0x3F;
				}
			}
			goto respond;
			break;
...
static int hidg_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	hidg_interface_desc.iInterface = us[status].id;
...
static struct usb_function *hidg_alloc(struct usb_function_instance *fi)
{
	...
	// Use feature report
	hidg->use_out_ep = 0;//!opts->no_out_endpoint;
